...
def main():
    df = criar_dataframe()
    respostas, valor_kwh = aplicar_questionario(df)
    consumo_atual, custo_atual = calcular_consumo_medio(df, respostas, valor_kwh)

    print(f"\nConsumo atual: {consumo_atual:.2f} kWh")
    print(f"Custo atual: R$ {custo_atual:.2f}")

    meta_consumo = definir_meta_consumo(consumo_atual)
    model, X_norm = criar_rede_neural(df, respostas, meta_consumo)
    novos_tempos = otimizar_consumo(model, X_norm, df, respostas, meta_consumo)

    novo_consumo, novo_custo = calcular_consumo_medio(df, novos_tempos, valor_kwh)
